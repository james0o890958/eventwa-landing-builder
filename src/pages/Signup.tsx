...
const Signup = () => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (token) {
        ...
        // Securely keep the credentials in sessionStorage temporarily
      ...
      // If the user tries to re-register with an email that already exists but isn't verified,
      // route them to the verification screen instead of showing a duplicate-email error.
      if (looksLikeDuplicateEmail && looksLikeUnverifiedAccount) {
        toast.error(
          "Email already registered. Please verify your email to continue."
        );
        navigate(
          `/verify-email?email=${encodeURIComponent(
            email
          )}&redirect=${encodeURIComponent(redirectTo)}&resend=true`
        );
        return;
      }
  ...
  const handleVerify = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      try {
        sessionData = await api.post("verify-otp", { email, otp });
      } catch (err: any) {
        if (err.message?.includes("404") || err.message?.includes("failed")) {
          sessionData = await api.post("verify-email", { email, otp });
        } else {
          throw err;
        }
      }
